...
const handleCategoryChange = async (selected) => {
  ...
  if (selected) {
    ...
    const exists = categories.some(
      (c) => c.value.toLowerCase() === selected.value.toLowerCase()
    );
    ...
    if (!exists) {
      try {
        ...
        const data = await apipost.post("/categories");
        const mapped = data.map((c) => ({
          value: c.name,
          label: c.name,
        }));
...
const handleCategoryKeyDown = async (event) => {
  if (event.key === "Enter" && inputValue) {
    ...
    const exists = categories.some(
      (c) => c.value.toLowerCase() === inputValue.toLowerCase()
    );
    ...
    if (!exists) {
      try {
        debugger; 
        // Save to DB
        await apipost.post("/products/categories/create", { name: inputValue });
        ...
        handleChange("category", inputValue);

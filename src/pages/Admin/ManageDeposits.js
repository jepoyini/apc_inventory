...
const DepositList = () => {
  ...
  document.title = "Manage Deposits | IBOPRO";
...
const columns = useMemo(
  () => [
...

    {
      ...
      cell: (cell) => {
        ...
        // Normalize coin type
        const coinType = (coin_type || 'flr').toLowerCase();
        ...
        const formattedAmountFlr = parseFloat(amount_flr || 0).toFixed(decimals);
        const formattedCoinValue = `${formattedAmountFlr} ${coinSuffix}`;

        // Set explorer link
        const link =
          coinType === 'flr'
            ? `https://flare-explorer.flare.network/tx/${transactionID}`
            : coinType === 'pol' || coinType === 'usdt'
  ...
  <div style={{ whiteSpace: 'pre-wrap', textAlign: 'left' }}>
    <strong>Amount (USD): </strong> ${parseFloat(amount || 0).toFixed(2)}<br />
    <strong>Coin Value: </strong> {formattedCoinValue}<br />
    <strong>Tx Hash: </strong>
    <a href={link} target="_blank" rel="noopener noreferrer" className="link-primary">
      {transactionID}
    </a><br />
    <strong>Sender Address: </strong> {senderWalletAddress}<br />
        ...
        <UncontrolledDropdown>
          ...
          <DropdownMenu className="dropdown-menu-end">
            <DropdownItem
              ...
              href="#"
              onClick={() => handleEditAction(cell.row.original.id,cell.row.original.amount,cell.row.original.amount_flr,cell.row.original.transactionID,cell.row.original.senderWalletAddress,cell.row.original.notes)}
            >
            ...
            <DropdownItem
              className="dropdown-item"
              href="#"
              onClick={() => handleAction("success", cell.row.original.id,cell.row.original.transactionID,cell.row.original.senderWalletAddress)}
            > &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                  Set Success
            </DropdownItem>
            <DropdownItem
  ...
  const handleModalSubmit = async () => {
    try {
      const url = "/updatedepositstatus";
      const obj = JSON.parse(sessionStorage.getItem("authUser"));
      const data = { id: CurrentID, status: "success", txtID: transactionID, senderAdd: senderAddress,uid:obj.id };
      const response = await api.post(url, data);
      console.log(response);
      if (response.success) {
  ...
  const handleEditModalSubmit = async () => {
    try {
      const url = "/updatedeposit";
      const obj = JSON.parse(sessionStorage.getItem("authUser"));
      const uid = obj.id;      
      const data = { id: CurrentID, USD: USD, FLR: FLR , txtID: transactionID, senderAdd: senderAddress, notes: Notes , uid: uid};
      console.log(data);
      const response = await api.post(url, data);
      debugger;
      if (response.success) {
  ...
  const handleAction = async (action, id,transactionid= '',serveraddress= '') => {
    ...
    if (action=="deleted")
    {
      ...
      try {
        const url = "/updatedepositstatus";
        const obj = JSON.parse(sessionStorage.getItem("authUser"));
        const data = { id: id, status: action,uid:obj.id };
        debugger; 
        const response = await api.post(url, data);
        if (response.success) {
  ...
  return (
    <div className="page-content">
      ...
      {loading ? (
            ...
            <Container fluid>
                <BreadCrumb title="Manage Deposits" pageTitle="Dashboard" />
              <Row>
                <Col lg={12}>
                  <Card id="customerList">
                    <CardHeader className="border-0">
                      <Row className="g-4">
                        <Col sm={4}>   <span className="table-caption">Manage Deposits</span></Col> 
                        <Col sm={8}>
                    ...
                    <CardBody className="pt-0">
                      ...
                      <Modal isOpen={editmodal} toggle={toggleEditModal} centered>
                        ...
                        <FormGroup>
                          ...
                          <FormGroup>
                            <Label for="txtUSD">USD Amount</Label>
                            <Input
                              type="text"
                              id="txtUSD"
                              value={USD}
                          ...
                          <FormGroup>
                            <Label for="txtFLR">COIN VALUE</Label>
                            <Input
                              type="text"
                              id="txtFLR"
                              value={FLR}

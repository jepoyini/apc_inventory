...
const PurchaseForm = () => {
  ...
  document.title = "Purchase | SHARING Studio Donation";
  ...
  const pricingData = Array.from({ length: 11 }, (_, i) => {
    const level = i + 1;
    const priceValue = 50 * Math.pow(2, i); // Doubles each level
    return {
      title: `Level ${level}`,
      price: `$${priceValue}`,
      description: `$${priceValue} Donation`,
      features: [
        `Entry into the Matrix Level ${level}`,
        `Level ${level} commission earnings`,
        `Exclusive Level ${level} training material`
      ],
      popular: level === 2 // Only Level 2 marked as "Popular"
    };
  });
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="SHARING Studio Donation" pageTitle="Dashboard" url="/dashboard/home" />
              ...
              <Row className="justify-content-center mt-5">
                <Col lg={8}>
                  <div className="text-center mb-4 pb-2">
                    <h4 className="fw-semibold fs-23">Choose the SHARING Studio Donation level that you want to avail</h4>

...
export default function SponsorTree() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (authUser) {
          const response = await api.post("/getsponsortree", { uid: authUser.id }, { showLoader: true });

          if (response.status === "success" && response.data) {
            setTreeData(response.data);
            setTotalMembers(countNodes(response.data));
            const expandedUpToLevel0 = collectExpandedNodes(response.data, 0, 0);
            setExpandedNodes(expandedUpToLevel0);
          } else {
            Swal.fire("Error", response.message || "Failed to load sponsor tree.", "error");
          }
        }
      } catch (err) {
        console.error(err);
  ...
  const handleSearch = () => {
    ...
    if (path) {
      ...
      setExpandedNodes([]);
      setSearchMatch(null);
      Swal.fire("Not Found", "No matching user ID, username, or name found in the sponsor tree.", "info");
    }
  };
  ...
  return (
    <div className="page-content">
      <Container fluid>
        <BreadCrumb title="Sponsor Tree" pageTitle="Dashboard" url="/dashboard" />
        <Row className="mb-3">
          <Col md={3}>
            <Input
              type="text"
